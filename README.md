...
- **Rich Authorization Requests (RAR — RFC 9396)**: fully wired end-to-end. Clients declare allowed `authorization_details` types via `AllowedAuthorizationDetailsTypes`; the authorize request validator parses `authorization_details` as a JSON array, requires every entry's `type` to be on the client's allowlist (a client with none configured can't use RAR at all, and one disallowed entry fails the whole request), carries the validated value through the authorization code, and echoes it on the token response. Scope: authorization code flow only, not client credentials/device flow
- **JARM (JWT Secured Authorization Response Mode)**: fully wired end-to-end. `response_mode=jwt`/`query.jwt`/`fragment.jwt`/`form_post.jwt` are accepted for the code/implicit/hybrid grants; the authorize endpoint signs the full response (code, state, tokens, etc.) as a JWT using the server's own signing credential and delivers it via query/fragment/form_post per the requested variant, so a client can cryptographically verify the response actually came from this server
- **FAPI 2.0 profile flag**: fully wired end-to-end. Per-client `RequireFapi2` composes PAR (implied even without separately setting `RequirePushedAuthorization`), `response_type=code`-only, PKCE with S256 mandatory (rejects `plain` even if the client otherwise allows it, and requires PKCE even for confidential clients), and DPoP-bound access tokens (a token request without a valid DPoP proof is rejected) into one flag. This is a useful subset of the FAPI 2.0 Security Profile, not full spec conformance — it doesn't restrict client authentication methods to asymmetric-only, doesn't support mTLS as an alternative to DPoP, and doesn't mandate JARM
