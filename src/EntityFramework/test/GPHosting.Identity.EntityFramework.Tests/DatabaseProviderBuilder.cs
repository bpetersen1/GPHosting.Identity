...
namespace GPHosting.Identity.EntityFramework.IntegrationTests
{
    /// <summary>
    ...
    public class DatabaseProviderBuilder
    {
        ...
        {
            var builder = new DbContextOptionsBuilder<T>();
            builder.UseSqlite($"Filename=./Test.GPHosting.Identity.EntityFramework-3.1.0.{name}.db");
            return builder.Options;
        }
        ...
        public static DbContextOptions<T> BuildLocalDb<T>(string name) where T : DbContext
        {
            ...
            builder.UseSqlServer(
                $@"Data Source=(LocalDb)\MSSQLLocalDB;database=Test.GPHosting.Identity.EntityFramework-3.1.0.{name};trusted_connection=yes;");
            return builder.Options;
        }
        ...
        {
            var builder = new DbContextOptionsBuilder<T>();
            builder.UseSqlServer($@"Server=(local)\SQL2016;Database=Test.GPHosting.Identity.EntityFramework-3.1.0.{name};User ID=sa;Password=Password12!");
            return builder.Options;
        }
    }
}